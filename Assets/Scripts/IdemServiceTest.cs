...
public class IdemServiceTest : MonoBehaviour
{
    ...
    private void OnGUI()
    {
        ...
        GUI.Label(new Rect(middleColumnX, 50, columnWidth, 30), "Is matchmaking: " + idemService.IsMatchmaking);
        GUI.Label(new Rect(middleColumnX, 80, columnWidth, 30), "Match found: " + idemService.CurrentMatchInfo.HasValue);
        GUI.Label(new Rect(middleColumnX, 110, columnWidth, 30), "Match ready: " + idemService.CurrentMatchInfo?.ready);
        GUI.Label(new Rect(middleColumnX, 140, columnWidth * 2f, 30), "Match id: " + idemService.CurrentMatchInfo?.matchId);
        GUI.Label(new Rect(middleColumnX, 170, columnWidth, 30), "Server: " + idemService.CurrentMatchInfo?.server);
        GUI.Label(new Rect(middleColumnX, 200, columnWidth, 30), "Game mode: " + idemService.CurrentMatchInfo?.gameMode);
        var y = 230;
